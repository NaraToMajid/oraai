...
// Chat state
let currentChat = [];
let chatHistory = JSON.parse(localStorage.getItem('oraai_history')) || [];
...
async function sendMessage() {
    ...
    // Prepare system prompt with all information
    const systemPrompt = `Anda adalah ORA-AI, asisten AI canggih yang dibuat oleh Oradev. 
    - Model: ORA-AI 1 (setara dengan ChatGPT 3)
    - Fungsi: Dirancang untuk pencarian cepat dan respons instan
    ...
    Selalu perkenalkan diri sebagai ORA-AI buatan Oradev di awal jawaban.`;
    
    try {
        const url = `https://api.siputzx.my.id/api/ai/gpt3?prompt=${encodeURIComponent(systemPrompt)}&content=${encodeURIComponent(userMessage)}`;
        ...
        let aiResponse = parsedData.response || parsedData.answer || 
                       ...
                       parsedData.message || data || "ERROR: NO RESPONSE";
        
        // Ensure ORA-AI introduction
        if (!aiResponse.includes("ORA-AI") && !aiResponse.includes("Oradev")) {
            aiResponse = "ORA-AI: " + aiResponse;
        }
        ...
        removeTyping();
        addMessage("ERROR: SYSTEM OFFLINE - ORA-AI SEDANG DALAM PERBAIKAN", false);
    } finally {
...
function saveHistory() {
    localStorage.setItem('oraai_history', JSON.stringify(chatHistory));
}
